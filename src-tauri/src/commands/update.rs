...
async fn fetch_latest_release_tag() -> Result<String, String> {
    if GITHUB_RELEASES_LATEST_API.is_empty() || GITHUB_RELEASES_LATEST_REDIRECT.is_empty() {
        return Err("github releases repository is not configured".to_string());
    }
    ...
    let api_response = api_client
        ...
        .header(header::USER_AGENT, "Open-Storyboard-Canvas-Updater")
        .send()
        .await
